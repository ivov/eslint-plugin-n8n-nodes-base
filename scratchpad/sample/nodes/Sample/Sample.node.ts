...
export class Sample implements INodeType {
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (resource === "all") {
					...
					throw new NodeOperationError(
						this.getNode(),
						`The resource '${resource}' is unknown!`,
						{
							itemIndex: i,
						}
					);
				...
				let responseData;
				if (returnAll) {
					responseData = await hackerNewsApiRequestAllItems.call(
						this,
						"GET",
						endpoint,
						qs
					...
					responseData = await hackerNewsApiRequest.call(
						this,
						"GET",
						endpoint,
						qs
