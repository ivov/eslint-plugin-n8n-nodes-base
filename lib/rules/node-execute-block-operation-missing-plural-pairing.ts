  ...
  create(context) {
    return {
      MethodDefinition(node) {
        ...
        const result = getOperationConsequents(node, { filter: "plural" });
        ...
        for (const operationConsequent of operationConsequents) {
          const lastStatement =
            operationConsequent.body[operationConsequent.body.length - 1];
          ...
          if (!isPluralPairingStatement(lastStatement, returnDataArrayName)) {
            ...
            context.report({
              ...
              node:
                getMarkedNodeFromConsequent(operationConsequent) ??
                operationConsequent,
